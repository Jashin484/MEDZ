...
class RCS: 
    ...
    def calculate(self, count_p, count_n = 50): 
        ...
        for n in range(count_n): 
            sum += ((-1)**(n + 1)) * ((n + 1) + 0.5) * (self.bn_function((n + 1), k) - \
                    self.an_function((n + 1), k)) 
...
def load_file(url): 
    ...
    path = Path("results", "task_rcs_02.json")
    ...
    return float(data['fmin']), float(data['fmax']), float(data['D'])


if __name__ == "__main__": 
    fmin, fmax, diameter = \
    load_file("https://jenyay.net/uploads/Student/Modelling/task_rcs_02.json") 
    count = 1000 
